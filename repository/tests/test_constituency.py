...
class ConstituencyTests(LocalTestCase):
    ...
    def test_get_constituency_for_date(self):
        ...
        c = get_constituency_for_date(
            name="A", date=datetime.date(year=1932, month=1, day=1)
        )
        self.assertEqual(c.parliamentdotuk, 1)

        # Within range
        c = get_constituency_for_date(
            name="A", date=datetime.date(year=1953, month=1, day=1)
        )
        self.assertEqual(c.parliamentdotuk, 1)

        # Within range
        c = get_constituency_for_date(
            name="A", date=datetime.date(year=1967, month=10, day=21)
        )
        self.assertEqual(c.parliamentdotuk, 2)

        # Within range
        c = get_constituency_for_date(
            name="A", date=datetime.date(year=1998, month=2, day=19)
        )
        self.assertEqual(c.parliamentdotuk, 3)

        # After most recent start which has no end (i.e. still active)
        c = get_constituency_for_date(
            name="A", date=datetime.date(year=2011, month=5, day=18)
        )
        self.assertEqual(c.parliamentdotuk, 4)

        # After latest end. Should return latest available.
        c = get_constituency_for_date(
            name="B", date=datetime.date(year=2021, month=6, day=5)
        )
        self.assertEqual(c.parliamentdotuk, 9)

        # Only one result for C. Should be returned regardless of requested date.
        c = get_constituency_for_date(
            name="C", date=datetime.date(year=1902, month=1, day=1)
        )
        self.assertEqual(c.parliamentdotuk, 10)

        # Only one result for C, should be returned regardless of requested date.
        c = get_constituency_for_date(
            name="C", date=datetime.date(year=2101, month=1, day=1)
        )
        self.assertEqual(c.parliamentdotuk, 10)

        c = get_constituency_for_date(
            "D", date=datetime.date(year=2101, month=1, day=1)
        )
    ...
    def test_get_suggested_constituencies(self):
        ...
        suggestions = get_suggested_constituencies(
            "aberdeen", datetime.date(1982, 3, 15)
        )
