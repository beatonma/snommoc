...
MEMBER_PORTRAIT_URL_PATTERN = re.compile(r'(https://members-api\.parliament\.uk/api/Members/[0-9]+/Portrait).*')
...
def update_member_portraits():
    ...
    members_without_portraits = Person.objects.filter(active=True, memberportrait__isnull=True)
    log.info(f'There are {members_without_portraits.count()} active members with no official portrait.')
