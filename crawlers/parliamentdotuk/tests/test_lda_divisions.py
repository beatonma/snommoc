...
class CommonsDivisionsTestCase(LocalTestCase):
    ...
    def test_create_commons_division(self):
        ...
        self.assertEqual(return_value, "The Queen's Speech: Jeremy Corbyn's amendment (c) ")
        ...
        self.assertEqual(division.title,  "The Queen's Speech: Jeremy Corbyn's amendment (c) ")
...
def get_mock_divisions(*args, **kwargs):
    url = args[0]
    if url == endpoints.COMMONS_DIVISIONS:
...
class MockCommonsDivisionTestCase(LocalTestCase):
    ...
    def test_update_commons_divisions(self):
        update_commons_divisions(follow_pagination=False)
        division = CommonsDivision.objects.first()

        self.assertEqual(division.title, "Opposition day debate on tax avoidance and evasion: Mr Corbyn's motion ")
