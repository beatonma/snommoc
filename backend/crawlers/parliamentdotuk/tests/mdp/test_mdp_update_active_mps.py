...
from crawlers.parliamentdotuk.tasks.membersdataplatform import active_members, schema
...
from repository.models.committees import CommitteeChair, CommitteeMember
...
class MdpUpdateActiveMpsTest(LocalTestCase):
    ...
    def test__update_house_membership(self):
        house_memberships = [
            schema.HouseMembership.model_validate(hm) for hm in SAMPLE_HOUSE_MEMBERSHIP
        ]
        ...
        self.assertEqual(lords_membership.start, date(year=2013, month=9, day=5))
        ...
        self.assertEqual(commons_membership.start, date(year=1974, month=2, day=28))
        self.assertEqual(commons_membership.end, date(year=1987, month=6, day=11))

    def test__update_historical_constituencies(self):
        historical_constituencies = [
            schema.Constituency.model_validate(c)
            for c in SAMPLE_HISTORICAL_CONSTITUENCIES
        ]
        ...
        result = ConstituencyResult.objects.get(
            constituency=stockton_south, start=date(year=1983, month=6, day=9)
        )
        self.assertEqual(result.election.name, "1983 General Election")
        self.assertEqual(result.election.date, date(year=1983, month=6, day=9))
        self.assertEqual(result.election.election_type.name, "General Election")
        self.assertEqual(result.end, date(year=1987, month=6, day=11))
    ...
    def test__update_basic_details(self):
        basic_info = schema.BasicInfo.model_validate(SAMPLE_BASIC_INFO)
    ...
    def test__update_committees(self):
        committees = [schema.Committee.model_validate(c) for c in SAMPLE_COMMITTEES]
        ...
        admin_committee_chair = CommitteeChair.objects.get(
            ...
            start=date(year=2017, month=11, day=6),
        )
        self.assertEqual(admin_committee_chair.end, date(year=2019, month=11, day=6))

    def test__update_declared_interests(self):
        interest_categories = [
            schema.DeclaredInterest.model_validate(c) for c in SAMPLE_INTERESTS
        ]
        ...
        self.assertEqual(durham_group_estates.created, date(year=2013, month=12, day=2))
        ...
        self.assertEqual(machine_delta.created, date(year=2015, month=12, day=31))
        self.assertEqual(machine_delta.amended, date(year=2018, month=6, day=21))

    def test__update_opposition_posts(self):
        opposition_posts = [
            schema.Post.model_validate(p) for p in SAMPLE_OPPOSITION_POSTS
        ]
    ...
    def test__update_parliamentary_posts(self):
        parliamentary_posts = [
            schema.Post.model_validate(p) for p in SAMPLE_PARLIAMENTARY_POSTS
        ]
    ...
    def test__update_government_posts(self):
        government_posts = [
            schema.Post.model_validate(p) for p in SAMPLE_GOVERNMENT_POSTS
        ]
    ...
    def test__update_addresses(self):
        addresses = [schema.Address.model_validate(a) for a in SAMPLE_ADDRESSES]
    ...
    def test__update_party_associations(self):
        parties = [
            schema.PartyMembership.model_validate(p) for p in SAMPLE_PARTY_ASSOCIATIONS
        ]
        ...
        self.assertEqual(labour.start, date(year=1974, month=2, day=28))
        self.assertEqual(labour.end, date(year=1981, month=3, day=2))
        ...
        self.assertEqual(libdem.start, date(year=1988, month=3, day=3))
        self.assertIsNone(libdem.end)
        self.assertEqual(libdem.party.parliamentdotuk, 17)

    def test__update_experiences(self):
        experiences = [schema.Experience.model_validate(e) for e in SAMPLE_EXPERIENCES]
        ...
        self.assertEqual(uk_land_estates.start, date(year=1995, month=12, day=25))
        self.assertEqual(uk_land_estates.end, date(year=2009, month=12, day=25))
    ...
    def test__update_elections_contested(self):
        ...
        contested = [
            schema.ContestedElection.model_validate(e)
            for e in SAMPLE_ELECTIONS_CONTESTED
        ]
        ...
        self.assertEqual(ge.election.date, date(year=1997, month=5, day=1))
    ...
    def test__update_subjects_of_interest(self):
        experiences = [
            schema.SubjectOfInterest.model_validate(e) for e in SAMPLE_BIOGRAPHY_ENTRIES
        ]
